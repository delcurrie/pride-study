    ...
    internal func drawDataSet(context context: CGContext?, dataSet: ScatterChartDataSet)
    {
        ...
        for (var j = 0, count = Int(min(ceil(CGFloat(entries.count) * _animator.phaseX), CGFloat(entries.count))); j < count; j++)
        {
            ...
            else if (shape == .Ring)
            {
                ...
                let outterRingSize : CGFloat = 4.0
                let innerRingSize : CGFloat = 2.0
                
                let path : CGMutablePathRef = CGPathCreateMutable()
                CGPathAddArc(path, nil, 0, 0, outterRingSize, 0, CGFloat(2) * CGFloat(M_PI), false)
                CGPathCloseSubpath(path)
                
                CGPathAddArc(path, nil, 0, 0, innerRingSize, 0, CGFloat(2) * CGFloat(M_PI), false)
                CGPathCloseSubpath(path)
